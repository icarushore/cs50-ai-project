...
def shortest_path(source, target):
    ...
    # TODO
    # Create a frontier to keep the nodes to visit
    frontier = QueueFrontier()
    node = Node(state=source,parent=None,action=None)
    frontier.add(node)
    # Create a set to keep the explored sets
    explored = set()

    while not frontier.empty():
        node = frontier.remove()    #pop it from the queue
        #if we have reached to the target
        if node.state == target:
            # Construct the path to return
            path = []
            while node.parent is not None:  # Backtrack to build the path
